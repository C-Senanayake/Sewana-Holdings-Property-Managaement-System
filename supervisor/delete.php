    ...
    if(!isset($_SESSION['user_ID'])){
        ...
        if(isset($_GET['user_id'])){
            ...
            if($user_id==$_SESSION['user_ID']){
                ...
                $quer="SELECT * FROM supervisor WHERE Supervisor_ID={$user_id} LIMIT 1";
                ...
                $query="DELETE FROM supervisor WHERE Supervisor_ID={$user_id} LIMIT 1";
                $result=mysqli_query($connection,$query);

                

                // $qery="DELETE FROM user WHERE U_ID={$user_id} LIMIT 1";
                // $rsult=mysqli_query($connection,$qery);
                if($result){
                    //user deleted
