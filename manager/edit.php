...
$user_id='';
    ...
    if(isset($_GET['user_id'])){
        $manager_id=mysqli_real_escape_string($connection,$_GET['user_id']);
        
        $query="SELECT * FROM manager WHERE Manager_No={$manager_id} LIMIT 1";
        ...
        if($result_set){
            if(mysqli_num_rows($result_set)==1){
                ...
                $appointed_date=$result['M_Appointed_Date'];
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($errors)){
            ...
            $query= "UPDATE manager SET M_Appointed_Date='{$Appointed_date}'";
            $query.="WHERE Manager_No={$manager_id} LIMIT 1";
...
<body>
    ...
    <form action="edit.php" method="POST">
        <input type="hidden" name="user_id" value="<?php echo $manager_id;?>"> 
