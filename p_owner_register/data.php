...
<?php
    ...
    if(isset($_GET['search'])){
        ...
        $query="SELECT * FROM property_owner_register WHERE (Owner_ID LIKE '%$search%' OR Branch_No LIKE '%$search%') ORDER BY Owner_ID";
    }else{
       
        $query="SELECT * FROM property_owner_register ORDER BY Owner_ID";
    }
    ...
    if($owners){
        while($owner=mysqli_fetch_assoc($owners)){
            ...
            $users_list.="<td class=\"t_data\">{$owner['Branch_No']}</td> ";
            if(($_SESSION['user_ID']<400000) ){
                $users_list.="<td class=\"t_data\"><a href=\"edit.php?user_id={$owner['Owner_ID']}\" >Edit</a> </td> ";
...
<body>
    ...
    <div class="search">
        <form action="data.php" method="GET">
            <input type="text" name="search" placeholder="Type Owner ID/Branch No & Press Enter" <?php echo $search; ?>autofocus>
        </form>
    </div>
