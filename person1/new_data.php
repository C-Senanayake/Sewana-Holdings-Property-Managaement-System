...
<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $password= $_POST['Password'];
         ...
         $req_fields=array('NIC','Name', 'Contact_No','Email','Address','Password');
        ...
        $max_len_fields=array( 'NIC'=>20,'Name'=>100, 'Contact_No'=>40,'Email'=>40,'Address'=>100,'Password'=> 20);
        ...
        if(empty($errors)){
            ...
            $query= "INSERT INTO person(Person_NIC,Person_Name,Person_Contact_No,Person_Email,Person_Address,Owner_ID,Passwor)";
            $query.="VALUES ('{$NIC}','{$Name}','{$Contact_No}','{$Email}','{$Address}','{$owner_id}','{$hashed_password}')";

            $result=mysqli_query($connection,$query);
            if($result){
                //query successful
                $user_id=mysqli_insert_id($connection);
                $query= "INSERT INTO user(U_ID,First_Name,NIC,passwor)";
                $query.="VALUES ('{$user_id}','{$Name}','{$NIC}','{$hashed_password}')";
...
<body>
    ...
    <h2 >Add New Person&ensp;&ensp;<span >
    <?php
    ...
    <input type="hidden" name="user_id" value="<?php echo $owner_id;?>">
         ...
         <div class="one_line"><div class="labels"><label for="E_name" name="Name">Person Name:</label></div>
         ...
         <div class="inputs"><input type="text" name="Email" id="input" placeholder="Email Address" <?php echo 'value="'. $email. '"';?>></div></div>
