...
$user_id='';
    ...
    $password='';
    if(isset($_GET['user_id'])){
        $nIC=mysqli_real_escape_string($connection,$_GET['user_id']);
        
        $query="SELECT * FROM person WHERE Person_NIC={$nIC} LIMIT 1";
        ...
        if($result_set){
            if(mysqli_num_rows($result_set)==1){
                ...
                $contact_No=$result['Person_Contact_No'];
                $email=$result['Person_Email'];
                $address=$result['Person_Address'];
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($errors)){
            ...
            $query= "UPDATE person SET Person_Name='{$Name}',Person_Contact_No='{$Contact_No}',Person_Email='{$Email}',Person_Address='{$Address}',Passwor='{$hashed_password}'";
            $query.="WHERE Person_NIC={$nIC} LIMIT 1";
...
<body>
    ...
    <form action="edit.php" method="POST">
        <input type="hidden" name="user_id" value="<?php echo $nIC;?>">
