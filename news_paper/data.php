...
<?php
    ...
    if(isset($_GET['search'])){
        ...
        $query="SELECT * FROM nws_paper WHERE (NP_Name LIKE '%$search%' OR NP_Contact_No LIKE '%$search%' NP_Type LIKE '%$search%' ) ORDER BY NP_Name";
    }else{
       
        $query="SELECT * FROM nws_paper ORDER BY NP_Name";
    }
    ...
    if($nw_ps){
        while($nw_p=mysqli_fetch_assoc($nw_ps)){
            ...
            $users_list.="<td class=\"t_data\">{$nw_p['NP_Contact_No']}</td> ";
            $users_list.="<td class=\"t_data\">{$nw_p['NP_Email']}</td> ";
            $users_list.="<td class=\"t_data\">{$nw_p['NP_Address']}</td> ";
            $users_list.="<td class=\"t_data\">{$nw_p['NP_Type']}</td> ";
            $users_list.="<td class=\"t_data\"><a href=\"edit.php?user_id={$nw_p['NP_Name']}\" >Edit</a> </td> ";
            $users_list.="<td class=\"t_data\"><a href=\"delete.php?user_id={$nw_p['NP_Name']}\" onclick=\"return confirm('Are You Sure?');\">Delete</a> </td> ";
            $users_list.="</tr>";
        }
    }else{
...
<body>
    ...
    <div class="search">
        <form action="data.php" method="GET">
            <input type="text" name="search" placeholder="Type Name/Contact No/Type & Press Enter" <?php echo $search; ?>autofocus>
        </form>
    </div>
    ...
    <table id="table">
        <tr id="t_row">
            ...
            <th id="t_header">N_P_Contact_Number</th>
            <th id="t_header">News Paper Email</th>
            <th id="t_header">News Paper Address</th>
            <th id="t_header">News Paper Type</th>
